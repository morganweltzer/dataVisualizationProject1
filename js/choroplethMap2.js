...
function updateChoropleth(updatedValue){
  ...
  class SecondChoroplethMap {
      ...
      vis.counties = vis.g.append("g")
        ...
        vis.counties
                  .on('mousemove', (d,event) => {
                    ...
                    // console.log(d);
                    // console.log(event);
                      const popPovPerc = d.properties.air_quality ? `<strong>${d.properties.air_quality}</strong> Air Quality <sup>2</sup>` : 'No data available'; 
                      d3.select('#tooltip')
                        .style('display', 'block')
    ...
    updateVis(){
      ...
      vis.colorScale = d3.scaleLinear()
          .domain(d3.extent(vis.data.objects.counties.geometries, 
            d => d.properties.myTestValue))
          .range(['#cfe2f2', '#0d306b'])
          .interpolate(d3.interpolateHcl);
  
      vis.counties
                  .attr('fill', d => {
                    if (d.properties.myTestValue) {
                      return vis.colorScale(d.properties.myTestValue);
                    } else {
                      return 'url(#lightstripe)';
                    }
                  });
